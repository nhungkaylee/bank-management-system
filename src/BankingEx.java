...
public class BankingEx {
	...
	public static void Menu() {
		System.out.println("\n 1.	Add the branch");
		System.out.println(" 2.	Edit the branch ");
		System.out.println(" 3.	Remove the branch");
	...
	public static void createAccount() throws SQLException, ClassNotFoundException {
		while (true) {
			...
			try {
				Connect con = new Connect();
				String query = "insert into dbo.account values ('" + customerId + "','" + accountId + "','" + branchId
						+ "', 0, 0)";
				con.statement.executeUpdate(query);
		...
		while (true) {
			...
			while (pmt.next()) {
				if (pmt.getString("accountId").equals(accountId)) {
					query = "update dbo.Account set customerId = '" + customerId + "', branchId = '" + branchId
...
//				
				...
				query = "update dbo.Account set balance = '" + currentAccount.getBalance() + "' where accountId = '"
						+ accountId + "'";
				con.statement.executeUpdate(query);
